...
int main(int argc, char **argv){
	...
	printf("window1\twindow2\tw_drift\tavgFaceTEMP\n");
	for(scale = 1; scale < 6; scale++) {
		printf("scale = %d\n", scale);
		for(window1 = 0.0; window1 < 1.1; window1 += .1){
			for(window2 = 0.0; window2 < 1.1; window2 += .1){
			...
			cudaEventRecord(start, 0);
				...
				for (iterCount =0; iterCount < ITERS * scale; iterCount++)
				{
